...
class GioHangRepo extends ConnectDB {
    ...
    function addGioHang($giohang) {
        ...
        $sql = "INSERT INTO giohang(ma_ctsp,ma_kh,gia_sp,so_luong) VALUES ('$ma_ctsp', '$ma_kh', '$gia_sp', '$so_luong')";
        $result = mysqli_query($this->conn, $sql);
        if ($result) {
            return true;
        }
    ...
    function deleteGioHang($ma_ctsp, $ma_kh) {
        $sql = "DELETE FROM giohang WHERE ma_ctsp='$ma_ctsp' AND ma_kh='$ma_kh'";
        $result = mysqli_query($this->conn, $sql);
        if ($result) {
            return true;
        }
    ...
    function deleteGioHangByMaKH($ma_kh) {
        $sql = "DELETE FROM giohang WHERE ma_kh='$ma_kh'";
        $result = mysqli_query($this->conn, $sql);
        if ($result) {
            return true;
        }
    ...
    function updateGioHang(GioHang $giohang) {
        ...
        $sql = "UPDATE giohang SET gia_sp='$gia_sp', so_luong='$so_luong' WHERE ma_kh='$ma_kh' AND ma_ctsp='$ma_ctsp'";
    ...
    function getSizeGioHang($ma_kh): int | null {
        try {
            ...
            $sql = "SELECT SUM(so_luong) AS total_quantity FROM giohang WHERE ma_kh='$ma_kh'";
    ...
    function getFullProduct($ma_ctsp) {
        try {
            $query = "SELECT * FROM sanpham sp JOIN chitietsanpham ctsp ON sp.ma_sp = ctsp.ma_sp WHERE ctsp.ma_ctsp = '$ma_ctsp'";
            $statement = mysqli_query($this->conn, $query);
