...
class PaginationRepo extends ConnectDB {
        ...
        try {
            if ($table == "chitietquyen") {
                $query = "SELECT DISTINCT chitietquyen.ma_quyen,chitietquyen.ma_chuc_nang FROM chitietquyen
                    JOIN nhomquyen ON chitietquyen.ma_quyen = nhomquyen.ma_quyen JOIN chucnangquyen 
                ...
                $query = "SELECT sp.*, ten_thuong_hieu, ten_loai, ten_hdh
                    FROM sanpham sp 
    ...
    public function getCount($table, $id) {
        try {
            if ($table == "chitietquyen") {
                $query = "SELECT count(DISTINCT nhomquyen.ma_quyen, chucnangquyen.ma_chuc_nang) AS num FROM chitietquyen 
                    JOIN nhomquyen ON chitietquyen.ma_quyen = nhomquyen.ma_quyen
                    JOIN chucnangquyen ON chitietquyen.ma_chuc_nang = chucnangquyen.ma_chuc_nang AND chucnangquyen.trang_thai = 0
                ...
                $query = "SELECT count(*) as num FROM khuyenmai WHERE tinh_trang LIKE '%$id%' AND trang_thai = 0";
            } else if ($table == "danhgia") {
